...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def collect_rollouts(
        ...
        replay_buffer_collection: Optional[bool] = False,
    ) -> bool:
        """
        ...
        """
        if replay_buffer_collection:
            self._last_obs = self.env.reset()

            callback = self._init_callback(callback, False)
        ...
        while n_steps < n_rollout_steps:
            if (n_steps+1)%10000 == 0:
                print(f"Collecting rollouts: {n_steps+1}/{n_rollout_steps}")
            ...
            if not replay_buffer_collection:
