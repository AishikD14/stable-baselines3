...
def d3rl_evaluation(model, exp_name, count):
    ...
    replay_buffer = np.load(f'discussion/{DIR}/iter_{str(count)}/replay_buffer.npz')
...
if __name__ == "__main__":
    ...
    SEARCH_INTERV = 1 # Since PPO make n_epochs=10 updates with each rollout, we can set this to 1 instead of 10
    NUM_ITERS = 1025600 // (args.n_steps_per_rollout*args.n_envs)
    N_EPOCHS = args.n_epochs
    ...
    if not normal_train:
        for i in range(START_ITER, NUM_ITERS, SEARCH_INTERV):
            ...
            print("Starting to collect rollouts")

            # Fill the replay buffer
            model.collect_rollouts(vec_env, None, model.rollout_buffer, n_rollout_steps=1000000, replay_buffer=model.replay_buffer, replay_buffer_collection=True)

            print("Replay buffer size: ", model.replay_buffer.observations.shape)

            # Save the replay buffer
            os.makedirs(f'discussion/{DIR}/iter_{str(count)}', exist_ok=True)
            np.savez(f'discussion/{DIR}/iter_{str(count)}/replay_buffer.npz',
                observations=model.replay_buffer.observations.reshape(-1, model.replay_buffer.observations.shape[-1]),
                actions=model.replay_buffer.actions.reshape(-1, model.replay_buffer.actions.shape[-1]),
                rewards=model.replay_buffer.rewards.reshape(-1, model.replay_buffer.rewards.shape[-1]),
                terminals=model.replay_buffer.dones.reshape(-1, model.replay_buffer.dones.shape[-1]),
                log_probs=model.replay_buffer.log_probs.reshape(-1, model.replay_buffer.log_probs.shape[-1]),
            )
            ...
            for j, a in enumerate(agents):
                ...
                returns_trains = evaluate_policy(model, dummy_env, n_eval_episodes=1000, deterministic=True)[0]
                print(f'avg return on 3 trajectories of agent{j}: {returns_trains}')
                cum_rews.append(returns_trains)

                # d3rl FQE evaluation code
                init_est = d3rl_evaluation(model, f"{'-'.join(DIR.split('/'))}", count)
