...
if __name__ == "__main__":
    ...
    if hasattr(args, 'max_grad_norm'):
        trpo_kwargs["max_grad_norm"] = args.max_grad_norm

    if hasattr(args, 'vf_coef'):
        trpo_kwargs["vf_coef"] = args.vf_coef

    if hasattr(args, 'clip_range'):
        trpo_kwargs["clip_range"] = args.clip_range

    if hasattr(args, 'learning_rate'):
        trpo_kwargs["learning_rate"] = args.learning_rate

    if hasattr(args, 'batch_size'): 
        trpo_kwargs["batch_size"] = args.batch_size

    if hasattr(args, 'normalize'):
        trpo_kwargs["normalize"] = args.normalize

    if hasattr(args, 'n_envs'):
        trpo_kwargs["n_envs"] = args.n_envs
    else:
        args.n_envs = 1

    if hasattr(args, 'sde_sample_freq'):
        trpo_kwargs["sde_sample_freq"] = args.sde_sample_freq

    if hasattr(args, 'ent_coef'):
        trpo_kwargs["ent_coef"] = args.ent_coef

    if hasattr(args, 'sub_sampling_factor'):
        trpo_kwargs["sub_sampling_factor"] = args.sub_sampling_factor

    if hasattr(args, 'cg_max_steps'):
        trpo_kwargs["cg_max_steps"] = args.cg_max_steps
    ...
    if normalize_kwargs:
        trpo_kwargs["normalize_kwargs"] = normalize_kwargs
    ...
    if not normal_train:
        for i in range(START_ITER, NUM_ITERS, SEARCH_INTERV):
            ...
            if avg_checkpoint:
                ...
                checkpoint_paths = [f'trpo_logs/{DIR}/models/agent{j}.zip' for j in range(1, 11)]
