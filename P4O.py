...
def policy_loss_evaluation(model):
    total_loss = []
    ...
    for rollout_data in model.rollout_buffer.get(model.batch_size):
        ...
        policy_loss_2 = advantages * torch.clamp(ratio, 1 - args.clip_range, 1 + args.clip_range)
...
if not normal_train:
    for i in range(START_ITER, NUM_ITERS, SEARCH_INTERV):
        ...
        for j, a in enumerate(agents):
            ...
            if not online_eval:
                loss = policy_loss_evaluation(model)
                print(f'avg policy loss on 5 trajectories of agent{j}: {loss}')
                advantage_rew.append(loss)
