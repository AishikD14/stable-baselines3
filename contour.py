...
if __name__ == "__main__":
    ...
    if not normal_train:
        for i in range(START_ITER, NUM_ITERS, SEARCH_INTERV):
            ...
            # Fit the checkpoints by Gaussian MLE
            mean, covariance = fit_gaussian_model(checkpoints)
            print("Mean of the Gaussian model:", mean)
            print("Covariance of the Gaussian model:", covariance)

            # Sample 1000 points from the fitted Gaussian distribution
            sample_points = np.random.multivariate_normal(mean, covariance, 1000)
            print("Shape of generated points:", sample_points.shape)

            # Save the sample points
            np.save(f'logs/{DIR}/sample_points_{i}_{i + SEARCH_INTERV}.npy', sample_points)

            agents = dump_weights(model.policy.state_dict(), sample_points)
            ...
            for j, a in enumerate(agents):
                ...
                print(f"Starting 1000 evaluations for agent{j}")
            ...
            print("Results saved")
            print("Shape of results:", cum_rews.shape)
